    ...
    def download_shapefile(self,
        ...
        if not os.path.isfile(local) and not os.path.isfile(e):
            ...
            try: 
                ...
                # Mengfei Mu 20180426
                # save url to file in landuse/.tif.url， download with curl in shell
                with open('{}/{}landuse.txt'.format(*its),'w') as f:
                    f.writelines(url)
                f.close()
                #
        ...
        for year in years:
            ...
            if not os.path.isfile(local) and year in available_years:
                ...
                try:
                    ...
                    with request.urlopen(url,context=context) as f: p = f.read().decode()
                except request.HTTPError as error:

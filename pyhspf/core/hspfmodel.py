...
class HSPFModel:
    ...
    def __init__(self, 
        ...
        # Mengfei Mu 20180611
        self.clouds          = {}  # cloud cover fraction
        #
    ...
    def add_timeseries(self, 
        ...
        # Mengfei Mu 20180611
        elif tstype == 'cloud':         d = self.clouds
        #
        else: 
    ...
    def build_wdminfile(self, verbose = False):
        ...
        # Mengfei Mu 20180611
        # add the cloud cover (fraction) data
        # use wind data now need to change later!
        attributes['TSFORM'] = 3
        attributes['TSTYPE'] = 'CLOU'
        attributes['IDCONS'] = 'CLOU'

        for t in self.clouds:

            start_date, tstep, data = self.clouds[t]
    ...
    def add_hydrology(self):
        ...
        for r in self.rchreses: 
            r.HYFG = True
            # Mengfei Mu 20180524
            # add HTRCH module for water temperature simulation
            r.HTFG = True
            #
    ...
    def ext_targets_block(self, 
        ...
        wdm = WDMUtil(verbose = verbose, messagepath = messagepath)
        #
        #wdm = WDMUtil(verbose = verbose, messagepath = self.messagepath)
...
#
        ...
        for tstype, identifier in self.watershed_timeseries.items():
            ...
            for otype, nmin, nmax in zip(otypes, nmins, nmaxs):
                ...
                if dtype is not None:
                    ...
                    # Mengfei Mu 20180606
                    # add EXT SOURCES for HTFG water temperature module
                    if dtype == 'SOLR':
                        otype = 'RCHRES'
                        v1 = (dsn, dtype, units, factor)
                        v2 = (otype, nmin, nmax, group, gtype)
    ...
    def ftables_block(self):
        ...
        for r in self.rchreses:
            ...
            for row in ftable:
                # Mengfei Mu 20180516
                # convert the complex to real
                if type(row[1]) == numpy.complex128:
                    for j in [0,1,2,3]:
                        row[j] = round(row[1].real,4)
                #

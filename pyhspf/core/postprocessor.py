...
class Postprocessor:
    ...
    def get_subbasin_timeseries(self, variables, comids, dates = None):
        ...
        for comid in comids:
            ...
            # print("values:",values,"areas:",areas)
    ...
    def get_obs_recession(self, comid, dates = None):
        ...
        # Mengfei 20180524
        # change zeros in flows to avoid error
        for n,i in enumerate(flows):
            if flows[n] == 0:
                flows[n] = (flows[n+1] + flows[n-1])/2.0
        #
    ...
    def get_involume(self, dates = None, tstep = 'daily'):
        ...
        for idcons, desc, dsn in zip(self.idconss, self.descriptions,
            ...
            # if (idcons == 'FLOW' and 
                ...
                # dsns.append(dsn)
        # Mengfei Mu 20180516
            if (desc in self.hspfmodel.inlets):
                dsns.append(dsn)
        # print('dsns:',dsns)
        #
        if len(dsns) == 0:
    ...
    def plot_calibration(self, comid = None, output = None, show = True, 
        ...
        # Mengfei Mu 20180524
        # avoid zero error
        for n,i in enumerate(d_oflow):
            if d_oflow[n] == 0 or d_sflow[n] == 0:
                try:
                    d_oflow[n] = (d_oflow[n-1] + d_oflow[n+1])/2.0
                    d_sflow[n] = (d_sflow[n-1] + d_sflow[n+1])/2.0
                except:
                    d_oflow[n] = d_oflow[n-1]
                    d_sflow[n] = d_sflow[n-1]
        #
